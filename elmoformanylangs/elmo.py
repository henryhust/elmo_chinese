...
def create_batches(x, batch_size, word2id, char2id, config, perm=None, shuffle=False, sort=False, text=None):
...
class Embedder(object):
    ...
    def sents2elmo(self, sents, output_layer=-2):
        read_function = read_list           # 函数对象
        ...
        for w, c, lens, masks, texts in zip(test_w, test_c, test_lens, test_masks, test_text):
            ...
            for i, text in enumerate(texts):
                ...
                if self.config['encoder']['name'].lower() == 'lstm':
                    ...
                    data = output[:, i, 1:lens[i]-1, :].data        # [layers, sen_id, sen_len, sent_dim]
        ...
        # print(after_elmo[0].shape, after_elmo)
        # after_elmo = recover(after_elmo, recover_ind)
        # print(after_elmo)
        after_elmo = numpy.array(after_elmo)
        return after_elmo
